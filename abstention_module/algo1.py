"""
Algo 1 — risques INCONDITIONNELS par recherche par DICHOTOMIE.

Transposition fidèle de la section 1 du notebook : une figure par métrique,
par fonction de confiance, par axe (PAS de panel multi-métriques illisible).

Pour chaque métrique (0/1 risk, FP risk, FN risk), avec ses propres `metric_targets`
et son propre `seed` de split, on produit via `sgp_metric_block` :
...
⚠️ MCD nécessite un modèle avec dropout (SmallCNN OK, GMIC non) : sur un run
sans sgp_set_mcd.pkl, seules les figures SR sont produites.

Usage :
    uv run python -m abstention_module.algo1
    uv run python -m abstention_module.algo1 <nom_du_run>
"""
...
theta_SR = (0.5, meta["kappa_max"])          # grille Sn-indépendante (SR)
theta_MCD = (-0.05, 0.0)                      # MCD = -variance (≤ 0)
MODE = "dicho"   # Algo 1 = dichotomie

# (label y, métrique, metric_targets, seed du split, sens du bound, slug fichier)
BLOCKS = [
    ("0/1 risk", "standard", np.linspace(0.03, 0.20, num_targets), 0, "<", "standard"),
    ("FP risk",  "FP",       np.linspace(0.03, 0.16, num_targets), 1, "<", "FP"),
    ("FN risk",  "FN",       np.linspace(0.001, 0.04, num_targets), 2, "<", "FN"),
]
