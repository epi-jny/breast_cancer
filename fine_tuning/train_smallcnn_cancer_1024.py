"""
SmallCNN v2 — cancer du sein (label malignant), images 1024×1024, optimisé H100.

Dérivé de train_smallcnn_cancer_v2.py mais :
- Target = cancer_label["malignant"] depuis data.pkl (pas le proxy "normalité" du CSV)
- Images 1024×1024 depuis cropped_1024/ (vs 512×512 cropped_512/)
- batch_size=64, num_workers=16, AMP, 100 epochs patience=15

Réutilise à l'identique l'architecture SmallCNNv2 (3 blocs conv + AdaptiveAvgPool(16×16)).

Usage:
    python -m fine_tuning.train_smallcnn_cancer_1024
    python -m fine_tuning.train_smallcnn_cancer_1024 --epochs 100 --batch-size 64
"""
...
def _make_entries(exam_list: list) -> list[tuple[str, int]]:
    """Aplatit la liste d'exams en (chemin_image, label_malignant).

    Label = exam["cancer_label"]["malignant"] (0/1), partagé par toutes les
    vues de l'exam — même logique que train_resnet.py._make_entries.
    """
    entries = []
    skipped_no_file = 0
...
# ─── Modèle : SmallCNN v2 (identique à train_smallcnn_cancer_v2.py) ──────────
...
class SmallCNNv2(nn.Module):
    """SmallCNN v2 — 3 blocs conv + AdaptiveAvgPool(16×16).
        ...
        AdaptiveAvgPool((16, 16))   # → 128×16×16 = 32768, indépendant de l'entrée
        Linear(32768, 512) → ReLU → Dropout → Linear(512, 1)
    """
    ...
    def forward(self, x):
        ...
        x = x.flatten(1)  # flatten (pas view) : robuste au format channels_last non-contigu
...
def train(
    ...
    stats_path = CHECKPOINT_DIR / f"train_stats_{img_size}_cancer_1024.json"
    t_start = time.time()
    ...
    # torch.compile optionnel (--no-compile pour désactiver). channels_last
    # retiré : incompatible avec l'entrée 1 canal (illegal memory access cudnn)
    # et sans bénéfice ici.
    if device == "cuda" and compile_model:
        print("torch.compile(model)… (1ère itération plus lente, compilation)")
    ...
    ax.set_title(f"ROC SmallCNNv2 cancer 1024px — val (epoch {ckpt['epoch']})")
    ax.legend()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SmallCNN v2 — cancer (malignant), 1024px, H100.")
    ...
    parser.add_argument("--no-compile",   action="store_true",
                        help="Désactive torch.compile (utile si erreur de compilation)")
