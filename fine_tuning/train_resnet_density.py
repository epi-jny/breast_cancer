"""
Entraînement ResNet18 sur RSNA — cible "density" (multi-classe BIRADS A/B/C/D).

Variante multi-classe des scripts `train_resnet*.py` :
...
Images sans density renseignée (NaN dans le CSV) sont exclues.

Usage:
    python -m fine_tuning.train_resnet_density --pretrained
    python -m fine_tuning.train_resnet_density --resume <run_dir>
"""
...
def compute_dataset_stats(
    ...
    if cache_path.exists():
        ...
        print(f"Stats chargées depuis {cache_path.name}")
...
def _make_sampler(entries: list) -> WeightedRandomSampler:
    """Pondération inverse des fréquences par classe pour équilibrer les batches."""
...
def _compute_class_weights(entries: list, device: str) -> torch.Tensor:
    """Poids inverses pour CrossEntropyLoss (normalisés à somme = NUM_CLASSES)."""
    labels = np.array([e[1] for e in entries])
...
def build_resnet18(device: str, pretrained: bool = False) -> nn.Module:
    ...
    # Spatial dropout sur les features avant pooling : drop des canaux entiers,
    # plus efficace que le dropout scalaire pour les CNN. Inséré entre layer4
    # et avgpool en remplaçant l'avgpool par Sequential(Dropout2d, avgpool).
...
def train(
    ...
    counts = np.bincount(list(label_lookup.values()), minlength=NUM_CLASSES)
    ...
    print(f"  train par classe : {dict(zip(CLASSES, train_counts.tolist()))}")
    print(f"  val   par classe : {dict(zip(CLASSES, val_counts.tolist()))}")
    print(f"Device: {device}  |  batch_size={batch_size}  |  img_size={img_size}")
    print(f"lr={lr}  |  weight_decay={weight_decay}  |  patience={patience}")
    print(f"pretrained={pretrained}  |  warmup={WARMUP_EPOCHS}\n")
    ...
    hyperparams = {
        ...
        "head_desc": f"Sequential(Dropout(p=0.5), Linear(512, {NUM_CLASSES}))",
    ...
    if resume_from is not None:
        ...
        best_f1 = state["best_auc"]  # réutilise le champ pour macro-F1
    ...
    # — Matrice de confusion du meilleur epoch
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Entraîne ResNet18 multi-classe sur la densité (A/B/C/D).")
